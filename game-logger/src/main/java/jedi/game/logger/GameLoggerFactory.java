...
class GameLoggerFactory {
	private GameLoggerFactory() {
	}

	private static String getAppenderName(GameLoggerType type, String fileName) {
		return "appender<" + type.name() + ">" + fileName;
	}

	private static String getLoggerName(GameLoggerType type, String fileName) {
	 ...
	 */
	private static void newLogger(GameLoggerType type, String fileDir, String fileName, String fileNamePrefix,
		...
		PatternLayout layout = PatternLayout.newBuilder().withConfiguration(JLoggerWriter.config)
				.withPattern(pattern)
				.withCharset(StandardCharsets.UTF_8)  // 设置编码为UTF-8
				.build();
		...
		// 滚动appender
		RollingFileAppender appender = RollingFileAppender.newBuilder().withAppend(true)
				.setConfiguration(JLoggerWriter.config)
				.withFilePattern(fileDir + fileNamePrefix + fileName + fileNameSuffix)
				.withPolicy(policy).withStrategy(strategy).withImmediateFlush(true).setLayout(layout)
	...
	private static void newDayLogger(String loggerDir, String loggerName, int deleteFileDay) {
		newLogger(GameLoggerType.DAY, loggerDir, loggerName, "%d{yyyyMMdd}_", ".log", "%d{yyyy-MM-dd HH:mm:ss},%m%n",
	...
	private static void newHourLogger(String loggerDir, String loggerName, int deleteFileDay) {
		newLogger(GameLoggerType.HOUR, loggerDir, loggerName, "%d{yyyyMMddHH}_", ".log", "%d{yyyy-MM-dd HH:mm:ss},%m%n",
	 ...
	 * @param loggerName
	 */
	static void stop(GameLoggerType type, String fileName) {
	...
	static Logger getHourLogger(String fileDir, String filerName) {
		String loggerName = getLoggerName(GameLoggerType.HOUR, filerName);
		synchronized (JLoggerWriter.config) {
	...
	static Logger getDayLogger(String fileDir, String filerName) {
		String loggerName = getLoggerName(GameLoggerType.DAY, filerName);
		synchronized (JLoggerWriter.config) {
			if (!JLoggerWriter.config.getLoggers().containsKey(loggerName)) {
	...
	static Logger getCustomCronLogger(String fileDir, String filerName, String fileNamePrefix,
			String fileNameSuffix, String pattern, String deleteFileTime) {
		String loggerName = getLoggerName(GameLoggerType.CUSTOM_CRON, filerName);
		synchronized (JLoggerWriter.config) {
			if (!JLoggerWriter.config.getLoggers().containsKey(loggerName)) {
				newLogger(GameLoggerType.CUSTOM_CRON, fileDir, filerName, fileNamePrefix, fileNameSuffix, pattern,
						deleteFileTime);
			}
		}
		return LogManager.getLogger(loggerName);
